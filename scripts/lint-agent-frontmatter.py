...
Rules (review 6.3):
  R1 error  name, description, model, tools all present; demoted to a
            ...
            (see tests/unit/test_reference_integrity.py)
...
def lint(path):
    """Lint a single agent markdown file against rules R1-R5.

    R1 and R2 are demoted from error to a "vendored: "-prefixed warning when
    the file is a symlink (vendored, upstream-owned frontmatter). R3-R5 apply
