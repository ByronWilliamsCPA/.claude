...
detect_keywords() {
    ...
    if echo "$text_lower" | grep -qE "\b(dockerfile|docker-compose|container|kubernetes|k8s|helm|pod|deployment)\b" && ! is_already_loaded "docker"; then
        suggestions="$suggestions docker"
        log "Suggesting docker tools based on keywords"
    fi

    # Browser/E2E testing patterns
    if echo "$text_lower" | grep -qE "\b(playwright|selenium|puppeteer|e2e|end.to.end|browser.test|ui.test|cypress)\b" && ! is_already_loaded "playwright"; then
        suggestions="$suggestions playwright"
        log "Suggesting playwright tools based on keywords"
    fi

    # Database patterns
    if echo "$text_lower" | grep -qE "\b(postgres|postgresql|mysql|database|sql|query|index|migration|schema|orm)\b" && ! is_already_loaded "postgres"; then
        suggestions="$suggestions postgres"
        log "Suggesting postgres tools based on keywords"
    fi

    # Error monitoring patterns
    if echo "$text_lower" | grep -qE "\b(sentry|error.monitor|exception|stack.trace|crash|bug.report|production.error)\b" && ! is_already_loaded "sentry"; then
        suggestions="$suggestions sentry"
        log "Suggesting sentry tools based on keywords"
    fi

    # Diagram patterns
    if echo "$text_lower" | grep -qE "\b(diagram|flowchart|sequence.diagram|class.diagram|uml|mermaid|plantuml|architecture.diagram|er.diagram)\b" && ! is_already_loaded "mermaid"; then
