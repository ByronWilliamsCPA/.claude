...
All configuration files have been created in `/home/user/cookiecutter-python-template/claude_config/`

### 1. **codecov.yml** - Code Coverage Configuration
- **Purpose**: Configure Codecov for coverage tracking and reporting
- **Cookiecutter Variables**:
  - `Claude Code Configuration` - Project name in comments
  - `Global Claude Code development standards, commands, and configuration for all projects` - Description in comments
  - `claude_config` - Module path for coverage tracking
...
- **Cookiecutter Variables**:
  - `claude_config` - Package name
  - `Byron Williams` - Copyright holder
  - `https://github.com/ByronWilliamsCPA/.claude` - Download location
...
- **Cookiecutter Variables**:
  - `Claude Code Configuration` - Site name and title
  - `Global Claude Code development standards, commands, and configuration for all projects` - Site description
  - `Byron Williams` - Author and copyright holder
  - `https://.claude.readthedocs.io` - Documentation site URL
  - `https://github.com/ByronWilliamsCPA/.claude` - Repository URL
  - `ByronWilliamsCPA` - GitHub org/user in repo name
  - `claude_config` - Project slug for repo name
- **Features**:
...
|----------|------|---------|---------|
| `Claude Code Configuration` | string | "My Python Project" | codecov.yml, mkdocs.yml |
| `claude_config` | auto | Derived from project_name | codecov.yml, renovate.json, REUSE.toml, mkdocs.yml, noxfile.py |
| `Global Claude Code development standards, commands, and configuration for all projects` | string | "A short description..." | codecov.yml, mkdocs.yml |
...
| `https://github.com/ByronWilliamsCPA/.claude` | auto | Derived from github info | REUSE.toml, mkdocs.yml |
| `https://.claude.readthedocs.io` | auto | Derived from project_slug | mkdocs.yml |
...
- **ingestion**: PDF/Image Ingestion module (`src/claude_config/ingestion/**`)
- **detection**: IQA Detection module (`src/claude_config/detection/**`)
- **correction**: Image Correction module (`src/claude_config/correction/**`)
- **output**: JSON Output module (`src/claude_config/output/**`)
- **utils**: Utilities module (`src/claude_config/utils/**`)
