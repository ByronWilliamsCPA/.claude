...
| ES-2 | Architecture decision with 2+ defensible options | Opus (plan-ceo-review / plan-devex-review) | Options with tradeoffs already written | "Which option, and what would change your mind?" |
...
| ES-12 | A gate or hook the docs promise appears to be missing or misfiring | User, and file an issue against this repo | Doctor output, expected vs observed | "Trust the doc or the runtime?" |

Application rule: once a stronger model or the user has answered an
escalation, record the answer in the task's working notes and do not
relitigate it in a later step of the same task. If new evidence contradicts
it, escalate again with the evidence; do not silently override.
