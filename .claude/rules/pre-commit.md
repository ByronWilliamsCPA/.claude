...
- [ ] **Snyk Secrets Gate**: If Snyk CLI is authenticated (`SNYK_TOKEN` is set or `snyk auth` has been run), run `snyk code test --detection-type=secrets .` before committing or pushing to scan the working tree for secrets. This closes the `--no-verify` bypass vector for the local trufflehog hook. Reference: `standards/snyk-mcp-setup.md`.
