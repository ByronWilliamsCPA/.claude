...
Implementation status: closed. `bash-pre-hook.sh` keys its `git reset --hard`
guard on the mutated branch as described above, and now parses `git checkout -B`
the same way: it blocks when the target ref being moved is protected, and allows
`git checkout -B <feature> main`, where the protected name is only the
start-point.
