...
Drafted 2026-08-02. Promoted to the global standards set 2026-08-02 from
`ByronWilliamsCPA/cyo-adventure` PR #562, per the file's own stated destination.

> **Relationship to `docs/standards-manifest.yaml`.** The manifest is a *check
> catalog*: it enumerates checks that exist. This spine is a *coverage
> denominator*: it enumerates the places a system can fail, so a gap shows up as
> a category with no checks rather than as a question nobody asked. They are
> complements. The manifest cannot tell you what it is missing; the spine can.
> Mapping between them lives in the `sp_category` field on manifest checks.
...
and a reassessment trigger for each N/A.
