...
```bash
gh api "repos/${REPO_SLUG}/branches/main/protection" --jq '.required_status_checks.contexts // []' 2>/dev/null
```
The four required blocking contexts are: `CI Gate`, `Security Gate Validation`, `Dependency & Standards Validation`, `Check REUSE Compliance`. If any of the four are absent from the returned array (or the endpoint returns 404), emit a CI-017 FINDING listing which contexts are missing and referencing `scripts/setup_github_protection.py` as the remediation script.
