...
via GitHub MCP `add_pull_request_review_comment` (or `create_pull_request_review`)
explaining what was done. Note: exact method name depends on the GitHub MCP
...
If yes: poll for CI status using `gh run list --branch {HEAD_BRANCH}` every
few minutes. When CI results arrive, if any fail, offer to run another
pr-fix pass.
