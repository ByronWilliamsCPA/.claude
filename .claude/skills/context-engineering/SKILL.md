...
> **Ported skill.** Adapted from [`addyosmani/agent-skills`](https://github.com/addyosmani/agent-skills/blob/main/skills/context-engineering/SKILL.md)
> (MIT License), commit `a5f0b17`, retrieved 2026-06-18. Adapted to our standards:
> em-dashes removed; rules-file guidance repointed to our scoped CLAUDE.md model
> (`CLAUDE.md` "Scoped context"); MCP guidance repointed to our tiered loading
> (`.claude/rules/mcp-strategy.md`, where Context7 is Tier 1 always-loaded); trust
> levels aligned with our untrusted-external-data directive (OWASP LLM01) and the
> session-length guidance in our root `CLAUDE.md`.
...
In our setup, CLAUDE.md operates at three scopes and the last scope wins on conflicts (see the "Scoped context" section of the root `CLAUDE.md`):

- Global: `~/.claude/CLAUDE.md` (universal standards)
- Project: `./CLAUDE.md` (repo-specific rules)
- Folder: `./src/CLAUDE.md` (conventions that differ from the parent scope; keep under 100 lines, only what differs)
...
- colocate tests next to source: `Button.tsx` then `Button.test.tsx`
...
For richer context, use Model Context Protocol servers. Our loading strategy is tiered (see `.claude/rules/mcp-strategy.md`): Context7 sits in Tier 1 (always loaded); the heavier servers below load on demand via agent bundles or keyword triggers.

| MCP Server | What It Provides | Our tier |
|-----------|-----------------|----------|
| **Context7** | Auto-fetches relevant documentation for libraries | Tier 1 (always) |
| **Chrome DevTools / Playwright** | Live browser state, DOM, console, network | Keyword-triggered |
| **PostgreSQL** | Direct database schema and query results | Keyword-triggered |
| **Filesystem** | Project file access and search | Built-in tools |
| **GitHub** | Issue, PR, and repository context | Tier 1 (file access) |

## Confusion Management

Even with good context, you will encounter ambiguity. How you handle it determines outcome quality. This reinforces our settings-and-permissions rule that "questions are not consent" and "silence is not consent": surface the ambiguity and wait for an explicit answer rather than guessing.
...
| "The context window is huge, I'll use it all" | Context window size is not attention budget. Focused context outperforms large context. |
