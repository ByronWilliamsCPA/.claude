...
When reviewing a branch created by an autonomous AI session, apply two additional checks before acting on its output:
...
candidate branch against main to identify what is genuinely salvageable, use
three-dot diff (`main...branch`) to see changes since the common ancestor, not
two-dot (`main..branch`), which conflates the two histories. Verify authorship of
