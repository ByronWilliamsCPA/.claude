...
1. **scope-analyzer** agent:
   - Task: "Analyze phase {N} scope against docs/IMPLEMENTATION_PLAN.md. Check which deliverables are DONE, PARTIAL, NOT_STARTED, or UNCLEAR. Detect any scope creep on the current branch. Return only a JSON object with this exact shape (no surrounding prose): {\"deliverables\": [{\"id\": \"<name>\", \"status\": \"DONE|PARTIAL|NOT_STARTED|UNCLEAR\", \"detail\": \"<one sentence>\"}], \"scope_creep\": [\"<item>\"], \"verdict\": \"PASS|FAIL\"}. Example: {\"deliverables\": [{\"id\": \"scope-analyzer\", \"status\": \"DONE\", \"detail\": \"agent implemented and tested\"}], \"scope_creep\": [], \"verdict\": \"PASS\"} If you cannot determine a deliverable's status for a genuine reason (missing tool, ambiguous scope), set its status field to UNCLEAR with the reason in detail rather than guessing a plausible-looking DONE. Do not fabricate a result to satisfy the JSON-only contract."
...
2. **phase-reviewer** agent:
   - Task: "Run quality gates for phase {N}. Execute: ruff check, ruff format --check, basedpyright, pytest with coverage, bandit. Check the per-phase smoke test from docs/IMPLEMENTATION_PLAN.md. Return only a JSON object with this exact shape (no surrounding prose): {\"gates\": [{\"name\": \"<check name>\", \"status\": \"PASS|FAIL|BLOCKED\", \"detail\": \"<one sentence>\"}], \"coverage_pct\": <float>, \"verdict\": \"PASS|FAIL|BLOCKED\", \"blocker\": \"<required when verdict is BLOCKED>\"}. Use BLOCKED when runtime observation is unavailable per `.claude/rules/testing.md`. Example: {\"gates\": [{\"name\": \"ruff check\", \"status\": \"PASS\", \"detail\": \"0 issues\"}], \"coverage_pct\": 87.4, \"verdict\": \"PASS\", \"blocker\": \"\"} If you cannot run a gate for a genuine reason (missing tool, ambiguous scope), set its status field to BLOCKED with the reason in detail rather than guessing a plausible-looking PASS. Do not fabricate a result to satisfy the JSON-only contract. For any smoke test or acceptance criterion that checks a control's behavior, confirm the assertion targets an effect independent of the control's own emitted claim (an external side effect, not a field the control itself writes and then reads back); a control can report success while doing nothing if the check only reads its own output."
