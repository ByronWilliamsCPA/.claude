...
**Baseline-vs-net-new discipline and suppression blast radius (Obs 788, 1148):** A literal "0 warnings" gate is unsatisfiable against a codebase that already carries structurally-unavoidable strict-mode warnings. `git stash` your changes, capture the baseline count/breakdown for the changed files, restore, and treat only net-new warnings as blocking; match the accepted idiom for warnings that merely replicate an existing pattern. When waiving a rule family in a per-file-ignore (e.g. `ANN` on `scripts/**`), check whether any retained rule depends on the waived family's type inference: PLW1514 (unspecified-encoding) needs an annotated receiver to fire, so waiving `ANN` silently disabled it and let an unencoded `Path.read_text()` ship, failing only on the Windows CI leg. A per-file-ignore's blast radius extends to every rule whose analysis depends on the waived one, not just the rules it names; always pass an explicit `encoding=` on text I/O regardless of rule coverage.
