...
* feat(hooks): extend `scripts/bash-pre-hook.sh` with four bypass-flag guards covering branch-protection and commit-hygiene controls; blocks `gh pr merge --admin` (CLI admin-merge), `gh api .../pulls/N/merge` (REST admin-merge; same effect when the caller has admin rights), `git ... --no-verify` (skips pre-commit/pre-push/commit-msg hooks; CLAUDE.md global rule), and `--no-gpg-sign` / `-c commit.gpgsign=<falsy>` / `-c tag.gpgsign=<falsy>` (bypasses the signed-commit requirement; matches case-insensitively per `git-config(1)` and accepts every falsy boolean: `false|0|no|off`); detection uses per-segment scanning (split on `&&`, `||`, `;`, `|`, subshell delimiters) so a bypass flag is blocked only when it belongs to the principal `git`/`gh` invocation, not when an unrelated tool elsewhere in a compound command shares the line; unwraps one level of `eval` / `bash -c` / `sh -c` / `zsh -c` indirection so flags inside the inner argument are still caught; blanks only the VALUE of `-m`/`--message` arguments before scanning (preserving flag-shaped tokens like `"--no-verify"` for the regex while ignoring commit-message documentation text); strips surrounding quotes from no-space tokens after message blanking so `git commit "--no-verify"` is treated as equivalent to its unquoted form (the shell strips those quotes before exec); uses `LC_ALL=C` to pin sed locale against multibyte input; logs every block with the full command for audit; pairs with the pre-existing force-push-to-main guard so all five hard-rule violations now exit with code 2 and a remediation-oriented message; registers the guard in source-of-truth `hooks.json` (not just live `~/.claude/settings.json`) so it survives `setup.sh` runs; ships `tests/scripts/test_bash_pre_hook_bypass_guards.sh` regression suite exercising 41 cases covering every bypass class identified during PR review; completes the half-built hook flagged in `feedback_orphan_safety_hooks.md`
