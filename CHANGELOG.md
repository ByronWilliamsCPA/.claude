...
* feat(hooks): extend `scripts/bash-pre-hook.sh` with three bypass-flag guards covering branch-protection and commit-hygiene controls; blocks `gh pr merge --admin` (bypasses required status checks; admin-merge is never an acceptable fix for failing CI), `git ... --no-verify` (skips pre-commit/pre-push/commit-msg hooks; CLAUDE.md global rule), and `--no-gpg-sign` / `-c commit.gpgsign=false` / `-c tag.gpgsign=false` (bypasses the signed-commit requirement enforced by every BW + williaby ruleset); strips single- and double-quoted strings before flag matching so a commit message containing literal "--admin" or "--no-verify" does not trigger a false positive; uses `LC_ALL=C` to pin sed locale against multibyte input; logs every block with the full command for audit; pairs with the pre-existing force-push-to-main guard so all four hard-rule violations now exit with code 2 and a remediation-oriented message; completes the half-built hook flagged in `feedback_orphan_safety_hooks.md`
