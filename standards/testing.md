...
> **Implementation**: Verification tools are provided by the cookiecutter-python-template.
> See project `scripts/` directory for actual implementations.

Ensure testing standards are consistently applied across repositories.

### Verification Tools (Project-Level)

| Script | Purpose | When to Run |
|--------|---------|-------------|
| `verify_test_structure.py` | Check required directories exist | Pre-push, CI |
| `check_test_ratios.py` | Enforce 70:20:10 pyramid | CI |
| `audit_test_coverage.py` | Find untested modules | Weekly, PR |
| `weekly_test_audit.py` | Full health report | Scheduled |
...
|-------|-----------|-----------|
| Pre-commit | New code has tests | 100% new files |
| Pre-push | Structure validation | Required dirs exist |
| CI | Ratio check + coverage | 80% line coverage |
| Weekly | Full audit report | Actionable items |
