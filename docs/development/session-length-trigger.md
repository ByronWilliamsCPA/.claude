...
1. **Handoff nudge (~150K carried tokens):** the proactive, lossless lever. At a task
   boundary past the cost knee, Claude offers `/compact` or `/handoff`. Operative text:
   root CLAUDE.md "Session length".
2. **Autocompact backstop (~375K):** the lossy fallback, set via
   `CLAUDE_CODE_AUTO_COMPACT_WINDOW=500000` + `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=75`.
...
- **Carry-ratio knee (the ~150K nudge signal).** The fill at which the smoothed carry
...
1. Read the carry-ratio knee distribution. If the median knee has moved, update the
   CLAUDE.md nudge floor (currently ~150K) to track it.
2. Review the peak-fill and compaction-count distributions. If most sessions now peak much
   higher or lower, revisit the ~375K autocompact backstop (and the
