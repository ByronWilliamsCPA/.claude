...
| `operations` | `operations-posture-auditor` | OPS-* (skipped when catalog `isDeployed` is not `true`) |
...
**OPS (Operations posture, 12 checks):** Runtime-only controls the six read-only OWASP specialist agents cannot statically verify by reading source: deployed runtime configuration attestation (OPS-001), data-store role documentation and row-level-security effectiveness proven in both directions (OPS-002/003), application log secret redaction proven by a test (OPS-004), security-relevant event logging against a documented taxonomy and test-fired alerting that reaches a human (OPS-005/006), inventoried automated backups and an actually-performed restore drill (OPS-007/008), anti-automation on public write paths and rate-limited authentication endpoints (OPS-009/011), runtime secrets sourced from a secret manager rather than baked into the artifact (OPS-010), and managed-service console settings versioned and pushed by CI rather than clicked (OPS-012). Every OPS check starts at `mechanism: unproven` and applies only to `deployed_repos` (catalog `isDeployed: true`); skipped for repos where that flag is not `true`. Evaluated by the `operations-posture-auditor` agent, the only OWASP-adjacent agent granted Bash to reach real runtime state.

**MKDOCS (MkDocs docs build, 14 checks):** mkdocs.yml metadata, theme, navigation, plugins, extensions, version pinning, docs/ structure, and CI build wiring. Skipped silently when `mkdocs.yml` is absent.
