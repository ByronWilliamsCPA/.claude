...
**4. Hook merge**: reads `hooks.json` from the repo root and union-merges it into the `.hooks` key of `~/.claude/settings.json`. Hook identity is the pair (group matcher, hook command). Entries defined in `hooks.json` are authoritative: their timeout and statusMessage values propagate on every run. Entries present only in `settings.json` (hooks registered by other installers, or event types that `hooks.json` does not define) are preserved, never deleted. Removing a hook from `hooks.json` does not remove it from a live `settings.json`; `--doctor` reports it as live-only drift for manual action. If `settings.json` does not exist, it is created from `hooks.json`. All other top-level keys (API keys, model preferences, `enabledPlugins`) are untouched, and a timestamped backup is created before any write.
